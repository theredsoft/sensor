...
```bash
# Для полной установки всех вариантов
bash fix_install.sh
# Выберите вариант 4 для автоматического поиска

# После установки запускайте
./run_simple.sh  # Простая версия (рекомендуется)
# или
./run.sh  # Если установлена полная библиотека
```

### ⚠️ Важно для новых версий Raspberry Pi OS (Bookworm и новее)

1. **Python 3.13+**: Оригинальная библиотека vl53l1x не компилируется. Используйте `vl53l1x_simple.py` - работает без проблем!
2. **PEP 668**: Система защищает системные пакеты Python. Используйте виртуальные окружения.
3. **Рекомендация**: Используйте `vl53l1x_simple.py` - минимум зависимостей, максимум надежности.
...
### Простой скрипт без зависимостей (vl53l1x_simple.py) - РЕКОМЕНДУЕТСЯ

Минималистичная версия с прямым доступом к I2C:

```bash
# Требует только smbus2
sudo venv/bin/python vl53l1x_simple.py
```

Особенности:
- Минимум зависимостей (только smbus2)
- Работает на любой версии Python
- Прямое управление I2C
- Не требует компиляции

### Оригинальный скрипт (vl53l1x_sensor_reader.py)
